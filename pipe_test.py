...
def main():
    ...
    for filename in os.listdir('test'):
        ...
        try:
            ...
            subprocess.run(args.bin.split(" "), stdin=open(f"test/{filename}"), stdout=open(f"temp_answer/{testname}.json", 'w'), timeout=5)
    ...
    for filename, content in res.items():
        ...
        # Compare only the final state (Architectural State)
        if rc[-1] != ra[-1]:
            print(f"Wrong answer for {filename}")
            print(f"Your answer (last state): \n{rc[-1]}")
            print(f"Correct answer (last state): \n{ra[-1]}")
            print("Diff:")
            print(diff_strings(pprint.pformat(rc[-1]), pprint.pformat(ra[-1])))
